...
RSpec.describe 'Uc3DmpId::Helper' do
  ...
  describe 'dmp_id_base_url' do
    ...
    it 'returns the ENV[\'DMP_ID_BASE_URL\']' do
      ...
      expect(described_class.dmp_id_base_url).to eql(ENV.fetch('DMP_ID_BASE_URL', nil))
    end
  ...
  describe 'landing_page_url' do
    ...
    it 'returns the ENV[\'DMP_ID_LANDING_URL\']' do
      ...
      expect(described_class.landing_page_url).to eql(ENV.fetch('DMP_ID_LANDING_URL', nil))
    end
  ...
  describe 'format_dmp_id(value:, with_protocol: false)' do
    ...
    it 'returns nil if the value is not a DOI' do
      expect(described_class.format_dmp_id(value: 'https://dmptool.org')).to be_nil
    end

    it 'removes the protocol from the value by default' do
      val = "#{ENV.fetch('DMP_ID_BASE_URL', nil)}/#{dmp_id}"
  ...
  describe 'path_parameter_to_pk(param:)' do
    ...
    it 'handles URI escaped characters' do
      result = described_class.path_parameter_to_pk(param: 'doi%3A11%2E2222%2F33333')
      expect(result).to eql('DMP#doi.org/11.2222/33333')
    end

    it 'replaces a domain with our base domain' do
      result = described_class.path_parameter_to_pk(param: 'doi:foo.bar/11.2222/33333')
      expect(result).to eql('DMP#doi.org/11.2222/33333')
    end
  ...
  describe 'dmp_id_to_pk(json:)' do
    it 'returns nil if :json is not a Hash' do
      expect(described_class.dmp_id_to_pk(json: 123)).to be_nil
    end

    it 'returns nil if :json does not contain :identifier' do
      expect(described_class.dmp_id_to_pk(json: JSON.parse({ foo: 'bar' }.to_json))).to be_nil
    end

    it 'returns nil if :format_dmp_id returns nil' do
      allow(described_class).to receive(:format_dmp_id).and_return(nil)
      expect(described_class.dmp_id_to_pk(json: JSON.parse({ identifier: '11.2222/12345' }.to_json))).to be_nil
    end

    it 'formats the PK as expected' do
      val = 'doi.org/11.2222/12345'
      allow(described_class).to receive(:format_dmp_id).and_return(val)
      result = described_class.dmp_id_to_pk(json: JSON.parse({ identifier: '11.2222/12345' }.to_json))
      expect(result).to eql("DMP##{val}")
    end
  end

  describe 'pk_to_dmp_id(p_key:)' do
    it 'returns nil if :p_key is nil' do
      expect(described_class.pk_to_dmp_id(p_key: nil)).to be_nil
    end
  ...
  describe 'parse_json(json:)' do
    ...
    it 'returns nil if :json is not a String or Hash' do
      expect(described_class.parse_json(json: 123)).to be_nil
    end
  ...
  describe 'eql?(dmp_a:, dmp_b:)' do
    ...
    it 'just compares the 2 values as-is if :dmp_a or :dmp_b is not a Hash' do
      expect(described_class.eql?(dmp_a: 'foo', dmp_b: { dmp: { bar: 'foo' } })).to be(false)
    end

    it 'just compares the 2 values as-is if :dmp_a or dmp_b does not have a top level :dmp' do
      expect(described_class.eql?(dmp_a: { foo: 'bar' }, dmp_b: { dmp: { bar: 'foo' } })).to be(false)
    end

    it 'returns true if :dmp_a and :dmp_b are identical' do
      expect(described_class.eql?(dmp_a: dmp, dmp_b: dmp)).to be(true)
    end
  ...
  describe 'extract_owner_id(json: {})' do
    ...
    it 'returns nil if :json is not a Hash' do
      expect(described_class.extract_owner_id(json: 123)).to be_nil
    end
  ...
  describe 'extract_owner_org(json: {})' do
    ...
    it 'returns nil if :json is not a Hash' do
      expect(described_class.extract_owner_org(json: 123)).to be_nil
    end
  ...
  describe 'cleanse_dmp_json(json:)' do
    ...
    it 'returns :json as-is if it is not a Hash or Array' do
      expect(described_class.cleanse_dmp_json(json: 123)).to be(123)
    end
    ...
    it 'returns the expected :dmproadmap_related_identifiers' do
      dmp = JSON.parse({
        ...
        dmproadmap_related_identifiers: [
          { work_type: 'output_management_plan', descriptor: 'is_metadata_for', identifier: 'http://skip.me',
            type: 'url' },
