...
class Responder
  ...
  class << self
    ...
    def respond(status: DEFAULT_STATUS_CODE, items: [], errors: [], **args)
      ...
      p "LambdaLayer: Responder.respond - #{e.message}"
      p " - STACK: #{e.backtrace}"
    ...
    def log_error(source:, message:, details: {}, event: {})
      ...
      p " - DETAILS: #{details.to_json}" if details.is_a?(Hash) && details.keys.any?
      p " - EVENT: #{event.to_json}" if event.is_a?(Hash) && event.keys.any?
    ...
    def _notify_administrator(source:, details:, event: {})
      ...
      payload += "CALLER RECEIVED: #{pp event.to_json}\n\n" if event.is_a?(Hash) && event.keys.any?
      payload += "DETAILS: #{pp details.to_json}\n\n" if details.is_a?(Hash) && details.keys.any?
      ...
      p "UNABLE TO NOTIFY ADMINISTRATOR! - #{e.message} - on #{source}"
      p " - EVENT: #{event.to_json}" if event.is_a?(Hash) && event.keys.any?
      p " - DETAILS: #{details.to_json}" if details.is_a?(Hash) && details.keys.any?
      nil
    end
