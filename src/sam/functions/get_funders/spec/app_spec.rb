...
RSpec.describe 'Functions::GetFunders' do
  ...
  describe '_results_to_response(term:, results:)' do
    ...
    it 'includes API info if applicable' do
      allow(described_class).to receive(:_weigh).and_return(1)
      recs = [results.last]
      items = described_class.send(:_results_to_response, term: 'api funder', results: JSON.parse(recs.to_json))
  ...
  describe '_weigh(term:, org:)' do
    ...
    it 'returns zero if :term is not a String' do
      expect(described_class.send(:_weigh, term: 123, org: org)).to eql(0)
    end
    it 'returns zero if :org is not a Hash' do
      expect(described_class.send(:_weigh, term: '123', org: 123)).to eql(0)
    end
    ...
    it 'applies the correct score when we have an acronym match' do
      expect(described_class.send(:_weigh, term: 'foo', org: JSON.parse(org.to_json))).to eql(1)
    end
    ...
    it 'applies the correct score when we have a partial name match' do
      expect(described_class.send(:_weigh, term: 'xample', org: JSON.parse(org.to_json))).to eql(1)
    end
    it 'applies the correct score when we have a starts with name match' do
      expect(described_class.send(:_weigh, term: 'exam', org: JSON.parse(org.to_json))).to eql(2)
    end
    it 'applies the correct score when we have an org_id' do
      org = { name: 'foo', org_id: 123 }
      expect(described_class.send(:_weigh, term: 'exam', org: JSON.parse(org.to_json))).to eql(1)
    end
    it 'is possible to have a score of zero' do
      org = { name: 'baz' }
      expect(described_class.send(:_weigh, term: 'exam', org: JSON.parse(org.to_json))).to eql(0)
    end
    it 'is possible to get a score of 3' do
      org[:acronyms] = '[\'exam\']'
      org[:org_id] = 123
      expect(described_class.send(:_weigh, term: 'xam', org: JSON.parse(org.to_json))).to eql(3)
    end
    it 'is possible to get a score of 5 (highest score)' do
      org[:acronyms] = '[\'exam\']'
      org[:aliases] = '[\'baz\', \'exam\']'
      org[:org_id] = 123
      expect(described_class.send(:_weigh, term: 'exam', org: JSON.parse(org.to_json))).to eql(5)
    end
  end
  ...
  describe '_establish_connection' do
    ...
    it 'calls SSM parameter store to fetch the credentials' do
      allow(Uc3DmpRds::Adapter).to receive(:connect).and_return(true)
      described_class.send(:_establish_connection)
      expect(Uc3DmpRds::Adapter).to have_received(:connect).with(username: 'foo', password: 'bar').once
    end
  end
end