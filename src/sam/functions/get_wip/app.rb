...
module Functions
  # The handler for: GET /wips/{wip_id+}
  class GetWip
    SOURCE = 'GET /wips/{wip_id+}'
    ...
    def self.process(event:, context:)
      ...
      # Query the DB
      wips = _select(owner: principal, wip_id: wip_id)
      return _respond(status: 404, items: [], event: event) unless !wips.nil? && wips.length.positive?

      _respond(status: 200, items: wips.map { |wip| wip['metadata'] }, event: event)
    ...
    class << self
      ...
      def _select(owner:, wip_id:)
        sql_str = <<~SQL.squish
          SELECT * FROM wips WHERE (metadata->>\'$.dmp.dmphub_owner_id\' = :owner_id)
            AND identifier = :wip_id LIMIT 1
        SQL
        Uc3DmpRds::Adapter.execute_query(sql: sql_str, owner_id: owner['id'], wip_id: wip_id)
      end
